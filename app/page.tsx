        ...
        <div className="w-full">
          {step === 1 && (
            <form onSubmit={(e) => { e.preventDefault(); if(firstName && lastName) setStep(2); }} className="flex flex-col items-center space-y-4">
              ...
              <input type="text" placeholder="FIRST NAME" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full p-3 border-[6px] border-black text-center text-2xl font-bold bg-white shadow-[8px_8px_0px_black] uppercase outline-none" />
              <input type="text" placeholder="LAST NAME" value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full p-3 border-[6px] border-black text-center text-2xl font-bold bg-white shadow-[8px_8px_0px_black] uppercase outline-none" />
          ...
          {step === 4 && (
            <div className="space-y-6 w-full animate-in zoom-in duration-500">
              {/* PARTY DETAILS BOX */}
              <div className="bg-[#03A9F4] border-[8px] border-black p-6 text-white shadow-[10px_10px_0px_black] text-center">
                 <p className="text-xl uppercase font-bold mb-4 italic leading-tight">
                   {firstName}, join us to celebrate Lucas' 5th Birthday!
                 </p>
                 <div className="text-left text-lg space-y-1 mb-4 italic font-bold border-y-2 border-white/20 py-3">
                 ...
                 <div className="text-sm font-bold uppercase tracking-tight italic">
                   <p className="mb-1">No gifts, please!</p>
                   <p>If you wish, donate to: <a href="https://charity.link" className="text-yellow-300 underline underline-offset-2">Spidey's Charity Link</a></p>
                 </div>
              </div>

              {/* RECEIPT CONFIRMATION */}
              <div className="bg-white p-6 receipt-shadow text-black font-mono relative">
                <div className="absolute top-0 left-0 w-full h-1 bg-[linear-gradient(90deg,transparent_50%,white_50%)] bg-[length:10px_10px]" />
                <h4 className="font-bold border-b border-dashed border-black pb-2 text-center uppercase mb-4 text-xl">Entry Confirmation</h4>
                <div className="text-sm space-y-2 uppercase">
                  ...
                  <div className="flex justify-between"><span>Squad:</span><span>{adults}A / {kids}K</span></div>
                  {kidNames.filter(n => n).length > 0 && (
                    <div className="pt-2 border-t border-dashed border-black">
                      <p className="text-[10px] text-gray-500 mb-1">Sidekicks:</p>
                      <p className="text-[11px] leading-tight">{kidNames.filter(n => n).join(', ')}</p>
                    </div>
                  )}
                </div>
                <div className="mt-6 pt-2 border-t-2 border-black flex justify-between font-bold text-lg">
              ...
              <div className="mt-10 flex flex-col items-center gap-3 w-full">
                ...
                {!emailSubmitted ? (
                  <form onSubmit={(e) => { e.preventDefault(); setEmailSubmitted(true); }} className="flex flex-col gap-3 w-full">
                    <input type="email" required placeholder="GUEST@EMAIL.COM" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-3 border-[6px] border-black text-center text-xl font-bold bg-white uppercase outline-none shadow-[6px_6px_0px_black]" />
