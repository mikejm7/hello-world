    ...
    <main className="relative w-full h-screen overflow-hidden flex flex-col items-center bg-[#FFEB3B]">
      ...
      {!isSwinging && step === 1 && (
        <div className="flex flex-col items-center justify-center h-full w-full">
          <div className="animate-pop relative -top-[10%] mb-12">
          ...
          <form onSubmit={(e) => { e.preventDefault(); if(firstName && lastName) triggerTransition(2); }} 
                className="flex flex-col items-center gap-4 w-64 relative top-[10%] animate-pop">
            <input required placeholder="FIRST NAME" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full p-2 border-4 border-black text-center text-lg font-bold bg-white shadow-[4px_4px_0px_black] uppercase outline-none" />
            <input required placeholder="LAST NAME" value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full p-2 border-4 border-black text-center text-lg font-bold bg-white shadow-[4px_4px_0px_black] uppercase outline-none" />
      ...
      {!isSwinging && step === 2 && (
        <div className="flex flex-col items-center justify-center h-full w-full px-4 animate-pop">
          ...
          <p className="text-2xl uppercase font-bold italic bg-white border-4 border-black px-6 py-1 shadow-[4px_4px_0px_black] transform rotate-1 mb-6">Join us to celebrate!</p>
          <div className="comic-panel p-4 w-full max-w-[320px] text-center mb-6">
          ...
          <div className="bg-white border-4 border-black p-4 shadow-[6px_6px_0px_black] text-center w-64">
        ...
        <form onSubmit={handleQtySubmit} className="flex flex-col items-center justify-center h-full w-full animate-pop">
          <div className="comic-panel p-6 w-full max-w-[320px]">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center"><label className="text-2xl">ADULTS:</label><input type="text" inputMode="numeric" required value={adults} onChange={(e) => setAdults(e.target.value)} className="w-16 h-12 text-center border-4 border-black text-2xl font-bold" /></div>
              <div className="flex justify-between items-center"><label className="text-2xl">KIDS:</label><input type="text" inputMode="numeric" value={kids} onChange={(e) => setKids(e.target.value)} className="w-16 h-12 text-center border-4 border-black text-2xl font-bold" /></div>
            </div>
      ...
      {!isSwinging && step === 5 && (
        <div className="flex flex-col items-center w-full h-full justify-center relative">
          <div className="animate-receipt-up overflow-hidden absolute bottom-[50%] flex flex-col justify-end z-10">
            <div className="bg-white w-[300px] p-6 receipt-font text-black border-x-4 border-t-4 border-black uppercase font-bold text-sm">
              ...
              <p className="text-center border-t-2 border-black mt-4 pt-2 italic text-lg">Thank You!</p>
              <div className="text-center text-[8px] mt-2">--- CONFIRMED ---</div>
            </div>
          </div>

          <div className="w-full h-2 bg-black absolute top-[50%] animate-shake z-20" />

          <div className="absolute top-[52%] flex flex-col items-center gap-2 animate-pop delay-1000">
             ...
             {!emailSubmitted ? (
               <form onSubmit={(e) => { e.preventDefault(); setEmailSubmitted(true); }} className="flex flex-col items-center gap-2">
                 <input type="email" required placeholder="GUEST@EMAIL.COM" value={email} onChange={(e) => setEmail(e.target.value)} className="w-64 p-2 border-4 border-black text-center font-bold bg-white uppercase outline-none" />
